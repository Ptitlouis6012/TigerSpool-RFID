...
{
    /* Este modulo PN532 volta a "power-down" entre comandos: sem este preambulo
       de wakeup, so o 1o comando apos o boot responde e os seguintes dao
       "Ack: Timeout". Acordar antes de CADA comando resolve. (~0.4ms) */
    _serial->write(0x55);
    _serial->write(0x55);
...
int16_t PN532_HSU::readResponse(uint8_t buf[], uint8_t len, uint16_t timeout)
{
    ...
    delay(10);  // era 100 - 10ms: rapido mas com margem p/ links HSU marginais
