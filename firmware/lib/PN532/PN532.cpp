...
uint8_t PN532::mifareultralight_ReadPage16 (uint8_t page, uint8_t *buffer16)
{
    pn532_packetbuffer[0] = PN532_COMMAND_INDATAEXCHANGE;
    ...
    // verifica o comprimento da resposta: status(1) + 16 bytes de dados.
    // Sem esta verificacao, uma resposta HSU truncada/timeout devolvia zeros
    // como se fosse leitura valida.
    int16_t n = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer));
    if (n < 17 || pn532_packetbuffer[0] != 0x00) {
        return 0;
    }
    memcpy (buffer16, pn532_packetbuffer + 1, 16);   // 4 paginas de uma vez
    return 1;
}
