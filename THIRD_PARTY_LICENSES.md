...
| [Noto Sans SC](https://github.com/notofonts/noto-cjk) | SIL OFL 1.1 | The Chinese interface. Medium weight, release tag `Sans2.004`: only the Han characters and CJK punctuation our own translations use - listed by `scripts/cjk-chars.py` - are merged into the `firmware/src/ui/font_ui_*.c` faces by `scripts/make-ui-font.sh`, not the face. Fetched and cached outside the tree; no font file is committed. |
