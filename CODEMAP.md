...
| `battery.cpp` | The ADC on GPIO5 measures the battery **rail**, not the battery, through a 200K/100K divider (so times three). On USB that rail is held up by the charger whether a cell is plugged in or not: a board with no battery reads 4.27 V, one with a half-charged cell reads 3.97 V (both measured). "Is there a battery" is therefore a threshold on the voltage, not a fact the hardware reports, and there is no charge-status line on this board - do not add a "charging" indicator without one. |
