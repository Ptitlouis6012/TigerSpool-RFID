...
def main() -> int:
    ...
    for src in sources:
        ...
        # The Chinese subset, merged in with --symbols (scripts/make-ui-font.sh).
        # Characters rather than a range, and just as much a fact about what the
        # face can draw.
        sm = re.search(r"--symbols\s+(\S+)", banner)
        symbols[src.name] = "".join(sorted(set(sm.group(1)))) if sm else ""
    ...
    distinct_symbols = sorted(set(symbols.values()))
    if len(distinct_symbols) > 1:
        print("the generated UI faces do not carry the same Chinese subset:",
              file=sys.stderr)
        for name, sym in sorted(symbols.items()):
            print(f"  {name}: {len(sym)} glyphs", file=sys.stderr)
        return 1

    OUT.write_text(json.dumps({
        "_comment": "GENERATED by scripts/gen-font-range.py - do NOT edit by hand.",
        "spec": distinct[0],
        "symbols": distinct_symbols[0] if distinct_symbols else "",
        "read_from": sorted(specs)[0],
        "sizes": len(specs),
    }, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(f"OK -> {OUT.relative_to(REPO)}   ({distinct[0]} + "
          f"{len(distinct_symbols[0]) if distinct_symbols else 0} CJK glyphs, "
